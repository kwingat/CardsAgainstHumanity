...
export class Dispatcher {
    ...
    on(typeOrCallbacks, callback = null, statusFilter = null) {
        if (_.isObject(typeOrCallbacks)) {
            /*
             If it's a type, loop over all the enumerable properties of that object
             then re-invoke this.on but dissect the type and the callback out from the
             property and value, but then map all the returned functions that are used to
             unsubscribe into the unreg variable. Then return a single function that
             invokes ever single on of those unsubscribe functions
             */
            const unreg = _.map(
                typeOrCallbacks,
    ...
    on$(type) {
        return new Observable(subscribe => {
            const unsub = this.on(type, value => subscribe.next(value));
        });
    }
